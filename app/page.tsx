...
export default function WalletApp() {
  ...
  const { prices, getPrice, formatPrice, get24hChange, loading: pricesLoading } = usePrices();
  const { contacts }                                                   = useContactBook(address);
  const { activity, loading: activityLoading, error: activityError, refetch: refetchActivity } = useActivity(address);
  ...
  const totalUsd = TOKENS.reduce((sum, t) => {
    const p = getPrice(t.symbol), b = balances[t.symbol];
  ...
  return (
    <div className="max-w-md mx-auto flex flex-col min-h-screen bg-bg">
      ...
      <div className="flex items-center justify-between px-4 pt-5 pb-3 flex-shrink-0">
        <div className="flex items-center gap-1.5 bg-surface border border-border1 rounded-full px-3 py-1.5">
        ...
        <button onClick={refetch}
          className="w-8 h-8 rounded-full bg-surface border border-border1 flex items-center justify-center text-muted hover:text-text transition-colors">
              ...
              <div className="bg-surface border border-border1 rounded-3xl px-5 py-5">
                <div className="flex flex-col items-center mb-5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <a href={celoscanAddress(address!)} target="_blank" rel="noreferrer"
          ...
          <div className="px-4 pt-4 space-y-3">
            ...
            <div className="bg-surface border border-border1 rounded-3xl p-5">
              ...
              <p className="text-sm text-muted leading-relaxed mb-5">
                Send CELO or any ERC-20 token to up to 50 addresses in a single transaction.
              </p>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[["Max wallets","50 / tx"],["Tokens","7 supported"],["Equal split","✓"],["Custom amounts","✓"]].map(([k,v]) => (
        ...
        {tab === "settings" && (
          <div className="px-4 pt-4 space-y-3">
            ...
            <p className="text-center text-[11px] text-muted pt-2">
              MiniWallet v0.1.0 · Powered by forno.celo.org<br />Prices via CoinGecko · Activity via CeloScan
            </p>
          </div>
        )}
      </div>
